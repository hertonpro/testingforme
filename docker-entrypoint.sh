...
" 2>/dev/null | grep -q "INIT" && {
    ...
    php -r "
        ...
        // Index
        \$pdo->exec('CREATE INDEX IF NOT EXISTS idx_tests_site ON tests(site)');
        \$pdo->exec('CREATE INDEX IF NOT EXISTS idx_tests_liaison_id ON tests(liaison_id)');
        \$pdo->exec('CREATE INDEX IF NOT EXISTS idx_tests_validation ON tests(validation)');
        ...
        echo 'OK';
    " 2>/dev/null && echo "✅ Base de données initialisée." || echo "⚠️  Erreur lors de l'initialisation."
} || {
    echo "✅ Base de données déjà initialisée."
}

# Supprimer install.php en production
...
echo "  Email : admin@testing-reseau.fr"
echo "  Mot de passe : \${ADMIN_PASSWORD:-admin123}"
echo "============================================"
echo ""
